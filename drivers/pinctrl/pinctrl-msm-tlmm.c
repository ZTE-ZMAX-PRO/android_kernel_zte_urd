...
static void msm_tlmm_gp_irq_resume(void)
{
	...
	spin_lock_irqsave(&ic->irq_lock, irq_flags);
     ...
     for_each_set_bit(i, ic->wake_irqs, num_irqs){
         msm_tlmm_set_intr_cfg_enable(ic, i, 0);
         //ZTE_PM_LHX_20160324 add to record which GPIO wakeups system begin ---->>
         if (msm_tlmm_get_intr_status(ic, i)) {
         ...
         //ZTE_PM_LHX_20160324 add to record which GPIO wakeups system end <<---- 
    }
...
static int gpio_debug_drv_set(void *data, u64 val)
{
	...
	drv_reg = val;//val >> 1;  //zte_pm_20151207, fixbug for HW wangqi output right current in zte_gpio
	//drv_reg -= 1;  //zte_pm_20151207, fixbug for HW wangqi output right current , zte_gpio
	if(drv_reg > 7)
		drv_reg	=	7;
    ...
    //writel_relaxed(val, cfg_reg);
    writel_relaxed(reg_val, cfg_reg);  //zte_pm_20151207, fixbug for HW wangqi output right current in zte_gpio
	return 0;
}
...
static int gpio_debug_drv_get(void *data, u64 *val)
{
	...
	*val = drv_reg;//(drv_reg + 1)*2; //zte_pm_20151207, fixbug for HW wangqi output right current in zte_gpio
	return 0;
}
...
// ZTE_chenjun
int msm_tlmm_gp_info_get_ext(uint pin_no, unsigned int *func, unsigned int *out, unsigned int *value)
{
	void __iomem *cfg_reg = TLMM_GP_CFG(tlmm_pininfo, pin_no);
	unsigned int cfg = readl_relaxed(cfg_reg);

	*func = (cfg >> TLMM_GP_FUNC_SHFT) & TLMM_GP_FUNC_MASK;
	*out = (cfg >> GPIO_OE_BIT) & 0x1;

	if (*out) {
	    *value = msm_tlmm_gp_out_get(pin_no, tlmm_pininfo);
	} else {
	    *value = msm_tlmm_gp_in_get(pin_no, tlmm_pininfo);
	}

	pr_err("gpio info:func(%#X), out(%#X), value(%#X)\n", *func, *out, *value);

	return 0;
}
EXPORT_SYMBOL(msm_tlmm_gp_info_get_ext);
//zte jiangfeng add, end
