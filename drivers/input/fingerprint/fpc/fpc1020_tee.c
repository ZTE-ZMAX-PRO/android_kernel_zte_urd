...
//		enable_irq(gpio_to_irq(fpc1020->irq_gpio)); //add by stone 2015_1211
...
//		disable_irq(gpio_to_irq(fpc1020->irq_gpio)); //add by stone 2015_1211
...
#if 1
//add by stone 2015_1222 for notify the fingerprint status to kernel
...
static ssize_t fp_msg_set(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct  fpc1020_data *fpc1020 = dev_get_drvdata(dev);

	if (!strncmp(buf, "acquired", strlen("acquired")))
	{
		fpc1020->fp_msg= FINGERPRINT_ACQUIRED;
		my_fp_msg=FINGERPRINT_ACQUIRED;
		dev_info(fpc1020->dev, "%s ---> %d \n", __func__,fpc1020->fp_msg);
		smp_wmb();
		pr_info("DBG fp_msg_set acquired\n");
		fb_blank_update_oem();
	}
	else if (!strncmp(buf, "enrolling", strlen("enrolling")))
	{
		fpc1020->fp_msg= FINGERPRINT_TEMPLATE_ENROLLING;
		my_fp_msg=FINGERPRINT_TEMPLATE_ENROLLING;
		dev_info(fpc1020->dev, "%s ---> %d \n", __func__,fpc1020->fp_msg);
		smp_wmb();
		pr_info("DBG fp_msg_set enrolling\n");
	}
	else if (!strncmp(buf, "removed", strlen("removed")))
	{
		fpc1020->fp_msg= FINGERPRINT_TEMPLATE_REMOVED;
		my_fp_msg=FINGERPRINT_TEMPLATE_REMOVED;
		dev_info(fpc1020->dev, "%s ---> %d \n", __func__,fpc1020->fp_msg);
		smp_wmb();
	}
	else if (!strncmp(buf, "authenticated", strlen("authenticated")))
	{
		fpc1020->fp_msg= FINGERPRINT_AUTHENTICATED;
		my_fp_msg=FINGERPRINT_AUTHENTICATED;
		dev_info(fpc1020->dev, "%s ---> %d \n", __func__,fpc1020->fp_msg);
		smp_wmb();
		pr_info("DBG fp_msg_set authenticated\n");
	}
	else
	{
		fpc1020->fp_msg= FINGERPRINT_ERROR;
		my_fp_msg=FINGERPRINT_ERROR;
		dev_info(fpc1020->dev, "%s ---> %d \n", __func__,fpc1020->fp_msg);
		pr_info("DBG fp_msg_set ERROR\n");
		return -EINVAL;
	}
...
static DEVICE_ATTR(fp_msg, S_IWUSR, NULL, fp_msg_set);
#endif
...
static int fpc1020_probe(struct spi_device *spi)
{
	...
	dev_err(dev, "slf1211-02  fpc1020_probe is enter...\n");
