...
#define NDT_FW_PATH			"/etc/firmware/pressure_dev_12.nfw"
...
/*ergate*/
#define FORCE_MAX_VALUE 512 //255
// ZTE_modify end
...
int ndt_stress_get_vlaue(unsigned int *value)
{
	...
	*value = gNdtData->custom_gain_value * (unsigned int) (gNdtData->value1)/100;
	...
	/*ergate*/
	*value = *value*256/FORCE_MAX_VALUE;
	if(gNdtData->value1>0 && *value < 1)
		*value = 1;
	...
	if(gNdtData->dmode == DEBUG_MODE_LOG)
		NDT_INFO("*************value1: %u value2: %u\n", gNdtData->value1,gNdtData->value2);
...
static void ndt_update_handle_work(struct work_struct *work)
{
	...
	/*ergate*/
		...
		NDT_ERROR("ndt_fwu_start_reflash() failed for other reasons!!!");
