...
#define SX9310_DBG2(fmt, args...)    //printk(SX9310_TAG  fmt, ##args)

static struct smtc_reg_data sx9310_i2c_reg_setup[] = {
	{
		.reg = SX9310_IRQ_ENABLE_REG,
		.val = 0x60,					//only enable far and close
	},
	{
		.reg = SX9310_IRQFUNC_REG,
		.val = 0x00,					//default setting, normal, irq, open-drain
	},
	{
		.reg = SX9310_CPS_CTRL1_REG,
		.val = 0x00,					//HZ during sleep mode and idle time
	},
	{
		.reg = SX9310_CPS_CTRL2_REG,
		//.val = 0x00,					//COMB not used, shiled
		.val = 0x04,					//COMB not used, shiled
	},
	{
		.reg = SX9310_CPS_CTRL3_REG,
		.val = 0x02,					//cs0 gain not used, set 0; cs1 gain, very important
	},
	{
		.reg = SX9310_CPS_CTRL4_REG,
		//.val = 0x0D,					//sampling frequency, precision, very important
		.val = 0x15,					//sampling frequency, precision, very important
	},
	{
		.reg = SX9310_CPS_CTRL5_REG,
		.val = 0xC5,					//start-up proximity detection cs1
		//.val = 0xC1,					//start-up proximity detection cs1
	},
	{
		.reg = SX9310_CPS_CTRL6_REG,
		.val = 0x20,					//thresholds which will trigger compensation
	},
	{
		.reg = SX9310_CPS_CTRL7_REG,
		.val = 0x4C,					//average debounce, filter strength
	},
	{
		.reg = SX9310_CPS_CTRL8_REG,
		.val = 0x0,						//cs0 detection threshold, not used
	},
	{
		.reg = SX9310_CPS_CTRL9_REG,
		//.val = 0x70,					//cs1 detection threshold
		.val = 0x86,					//cs1 detection threshold
	},
	{
		.reg = SX9310_CPS_CTRL10_REG,
		.val = 0x00,					//average freezing, HYST, debounce
	},
	{
	...
	},
	{
		...
		.val = 0x00,					//reference sensor calibration value
	},
	{
	...
	},
	{
		...
		//.val = 0x04,					//reference sensor correction
		.val = 0x00,					//reference sensor correction
	},
	{
		.reg = SX9310_CPS_CTRL18_REG,
		.val = 0x00,					//cs0,COMB reference coefficient
	},
	{
		.reg = SX9310_CPS_CTRL19_REG,
		.val = 0x00,					//cs1, cs2 reference coefficient
	},
	{
		.reg = SX9310_SAR_CTRL0_REG,
		.val = 0x00,					//sar control, default value
	},
	{
		.reg = SX9310_SAR_CTRL1_REG,
		.val = 0x80,					//sar control, default value
	},
	{
		.reg = SX9310_SAR_CTRL2_REG,
		.val = 0x0C,					//sar control, default value
	},
	{
		.reg = SX9310_CPS_CTRL0_REG,
		.val = 0x52,			//scan period 0x5, cs1, when set will auto compensate
	},
	{
		.reg = SX9310_CPSRD,
		.val = 0x1,			//scan period 0x5, cs1, when set will auto compensate
	},
};
...
#define SX9310_ACTIVE "sx9310_active"
#define SX9310_SLEEP "sx9310_sleep"
...
static void sx93XX_schedule_work(psx93XX_t chip, unsigned long delay)
{
  ...
  if (chip) {
   ...
   //after waiting for a delay, chip put the job in the kernel-global workqueue. so no need to create new thread in work queue.
...
static int sx9310_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  if (chip)
  {
    ...
    else
    {
      ...
      {
        printk("%s, error pinctrl_lookup_state() for SX9310_ACTIVE\n", __func__);
        goto probe_error;
      }
    ...
    if (IS_ERR(chip->sx9310_vdd)) {
      printk("unable to get sx9310 vdd\n");
    ...
    if (IS_ERR(chip->sx9310_vdd)) {
      printk("unable to get sx9310 svdd\n");
