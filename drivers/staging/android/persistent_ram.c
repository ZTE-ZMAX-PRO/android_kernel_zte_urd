...
/* incr ease the size counter, retuning an error if it hits the max size */
        ...
        const void *s, unsigned int start, unsigned int count)
{
        struct persistent_ram_buffer *buffer = prz->buffer;
        //size_t start_pos;
        char *dst =(char *)(buffer->data) + start;
        const char * src = (const char *)s;
        //size_t add =(size_t)(buffer->data);
        //add += start;
        //dst = (char *)add;

        //start_pos = DIV_ROUND_UP(start,(sizeof(size_t)));
        //start_pos  = ((start + sizeof(size_t) -1)/sizeof(size_t))*sizeof(size_t);
        while (count--) {
                *(char *)dst = *(char *)src;
                dst = (char *)dst + 1;
                src = (char *)src + 1;
	...
	const void *s, unsigned int count)
{
	...
	rem = prz->buffer_size - start;
        ...
        if (unlikely(rem < c)) {
                persistent_ram_update(prz, psrc, start, rem);
        ...
        persistent_ram_update(prz, psrc, start, c);
...
{
/*
 * RAM console support by ZTE_BOOT 20140929
 */
#ifdef CONFIG_ZTE_RAM_CONSOLE
	if (offset_in_page(start) != 0) {
		pr_warning("phys address isnot align with page!\n");
	}
	...
	pr_info("ram_console: got buffer's virt address at 0x%p, size 0x%lx\n", prz->buffer, (long)prz->buffer_size);
	return 0;
#else
	struct page **pages;
...
#ifdef  ZTE_64BIT
        atomic64_set(&prz->buffer->start, 0);
        atomic64_set(&prz->buffer->size, 0);
#else
