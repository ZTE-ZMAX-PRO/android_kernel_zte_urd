...
static int spk_amp_get(struct snd_kcontrol *kcontrol,
		       struct snd_ctl_elem_value *ucontrol)
{
	pr_err("%s: chenjun: msm8952_spk_control(%d)\n",
			 __func__, msm8952_spk_control);
	ucontrol->value.integer.value[0] = msm8952_spk_control;
	return 0;
}
		       ...
		       struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_err("%s: chenjun: msm8952_spk_control(%d), put val(%ld)\n",
	...
	{
		ret = msm_gpioset_activate(CLIENT_WCD_EXT, "spk_amp_gpio");
		if (ret < 0)
		{
	...
	{
		ret = msm_gpioset_suspend(CLIENT_WCD_EXT, "spk_amp_gpio");
		if (ret < 0)
		{
...
// chenjun
	SOC_ENUM_EXT("Spk Amp Switch", msm_snd_enum[0], spk_amp_get,
			spk_amp_put),
//
};
...
static int msm8952_ak4961_codec_up(struct snd_soc_codec *codec)
{
	int err;
	bool timedout;
		...
		enum ak49xx_codec_event codec_event)
{
	...
	case AK49XX_CODEC_EVENT_CODEC_UP:
		return msm8952_ak4961_codec_up(codec);
	default:
...
int msm_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	ak4961_event_register(msm8952_codec_event_cb, rtd->codec);
#endif
	codec_reg_done = true;
	return 0;
out:
...
static int msm8952_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		...
		// ZTE_chenjun:goto err;
	}

// ZTE_chenjun
// Disable spk amp by default
	ret = msm_gpioset_activate(CLIENT_WCD_EXT, "spk_amp_gpio");
	if (ret < 0)
	{
	    pr_err("%s: gpio set cannot be activated %s\n",
