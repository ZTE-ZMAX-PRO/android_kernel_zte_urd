...
static unsigned int AK4375_LDO_GPIO= 7;//default
...
static int ak4375_set_bickfs(struct snd_soc_codec *codec)
{
	...
	if ( ak4375->nBickFreq == 0 ) { 	//32fs
		...
		ak4375_writeMask(codec, AK4375_15_AUDIO_IF_FORMAT, 0x02, 0x02);	//DL1-0=00(24bit, >=64fs)
	}
	else {								//64fs
...
struct snd_ctl_elem_value  *ucontrol)
{
	...
	printk("[LHS] %s ak4375->nBickFreq=%d\n",__func__,ak4375->nBickFreq);
...
*/
		 ...
		 pr_err("%s zjb  ldoen_gpio : %d\n",__func__,AK4375_LDO_GPIO);	 
		 if (AK4375_LDO_GPIO < 0) {
			 dev_err(dev, "Looking up zjb failed %d\n",
				AK4375_LDO_GPIO);
			 //goto err;
		 }
		 ...
		 if (AK4375_LDO_GPIO)
		 {
		   ...
		   if (ret) {
			   pr_err("%s: zjb Failed to request gpio AK4375_LDO_GPIO %d\n", __func__,
				   AK4375_LDO_GPIO);
			   // ak49xx->ldoen_gpio = 0;
			   // return ret;
		   }
		 ...
		 AK4375_RESET_GPIO = of_get_named_gpio(dev->of_node,
					 "akm,cdc-reset-gpio", 0);
		 pr_err("%s zjb  AK4375_RESET_GPIO : %d\n",__func__,AK4375_RESET_GPIO);	 

		 if (AK4375_RESET_GPIO < 0) {
			 dev_err(dev, "Looking up %s property in node failed %d\n",
		 ...
		 if (AK4375_RESET_GPIO)
		 {
		   ...
		   if (ret) {
			   pr_err("%s: zjb Failed to request gpio  AK4375_RESET_GPIO %d\n", __func__,
				   AK4375_RESET_GPIO);
			   // ak49xx->ldoen_gpio = 0;
			   // return ret;
		   }
         ...
         if(false){
			 ...
			 if(ret)
			 printk("zjb 1-------AK4375_RESET_GPIO  0");
             msleep(30); // chenjun:orig:100
         }
         ...
         if(AK4375_LDO_GPIO){
			 ...
			 if(ret)
			 printk("zjb 2-------AK4375_LDO_GPIO  1");
             msleep(30); // chenjun:orig:100
         }
         ...
         if(AK4375_RESET_GPIO){
			...
			if(ret)
			printk("zjb 1- ------AK4375_RESET_GPIO  0");
			msleep(20); // chenjun:orig:100
             ret = gpio_direction_output(AK4375_RESET_GPIO, 1);
			 if(ret)
			 printk("zjb 3-------AK4375_RESET_GPIO  1");
             msleep(20); // chenjun:orig:100
         }
...
struct snd_ctl_elem_value  *ucontrol)
{
	...
	for ( i = regs ; i <= rege ; i++ ){
		...
		printk("[LHS]***AK4375 Addr,Reg=(%x, %x)\n", i, value);
	}
...
static int ak4375_set_pllblock(struct snd_soc_codec *codec, int fs)
{
	...
	snd_soc_write(codec, AK4375_05_CLOCK_MODE_SELECT, mode);
	pr_err("LHS ak4375_data->nBickFreq=%d",ak4375_data->nBickFreq);
	if ( (fs % 8000) == 0 ) {
		nPLLClk = 122880000;
	}
...
/*	else if ( ak4375_data->nBickFreq == 1 ) {	//48fs
	...
	PLMbit--;
	MDIVbit--;
	pr_err("LHS PLDbit=%d PLMbit=%d  MDIVbit=%d\n",PLDbit,PLMbit,MDIVbit);
	//PLD15-0
	snd_soc_write(codec, AK4375_0F_PLL_REF_CLK_DIVIDER1, ((PLDbit & 0xFF00) >> 8));
	snd_soc_write(codec, AK4375_10_PLL_REF_CLK_DIVIDER2, ((PLDbit & 0x00FF) >> 0));
	//PLM15-0
		...
		struct snd_soc_dai *dai)
{
	...
	{
		ak4375_writeMask(codec, AK4375_15_AUDIO_IF_FORMAT, 0x03, 0x01);
		ak4375_data->nBickFreq = 0;
		akdbgprt("\t[AK4375]LHS %s:format= 16bit\n",__FUNCTION__);
	}
	else if(format == SNDRV_PCM_FORMAT_S24_LE)
	{
		ak4375_writeMask(codec, AK4375_15_AUDIO_IF_FORMAT, 0x02, 0x02);
	                ...
	                struct device_attribute *attr, char *buf)
{
	...
	for ( i = 0 ; i <= AK4375_MAX_REGISTERS ; i++ )
		{
			ret=snd_soc_read(ak4375_codec,i);
			if(ret<0){
				printk("Read AK4375 register error\n");
				}
		rx[i]=ret;	
		}
	if(i == 25){
...
{	
	char *ptr_data = (char *)buf;
	 ...
	 {	
		...
		printk("AK4375 write add=%d, val=%d.\n",val[i],val[i+1]);
	 }
	 return count;
}
...
//	akdbgprt("\t[AK4375] %s(%d) ak4375=%x\n",__FUNCTION__,__LINE__, (int)ak4375);
	...
	if(ret)
	{
		printk("AK4375 ERROR to create reg_data\n");
	}
#endif

	printk("[LHS AK4375 zjb11] (success)\n");
	return ret;
}
                            ...
                            const struct i2c_device_id *id)
{
	...
	dev_set_name(codec->dev,"ak4375");
	ak4375_ldo_en(codec->dev);
