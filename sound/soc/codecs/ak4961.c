...
static int ak49xx_bandswitch_set(struct snd_soc_codec *codec,bool howtochange);
...
struct ak4961_priv {
	...
	enum   ak4961_slimbus_stream_state stream_state;
       enum   ak4961_slimbus_stream_state pcm_state; //add for band set
...
static void ak4961_work(struct work_struct *work)
{
	...
	case AK4961_DSPRSTNON:
            if (ak4961->pcm_state == AK4961_SLIMBUS_STREAM_ON)
        		...
        		if((ak4961_dsp_mode>=DSP_MODE_NARROW_HANDSET)&&(ak4961_dsp_mode<=DSP_MODE_WIDE_HANDSFREE))
        			{
        ...
        struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ret == 0) {
		...
		ak4961_band_set=0;//flag the band set state
...
static int ak49xx_bandswitch_set(struct snd_soc_codec *codec,bool howtochange)
{
		...
		mir4_3=snd_soc_read(codec, MIR4_REGISTER_3);
    		pr_err("[lhs] %s mir3_2=%x , mir4_3= %x\n",__func__,mir3_2,mir4_3);             
        if(howtochange==1) // if N to W 
                { 
		   val[0] = 0x81; 
                val[1] = RUN_STATE_DATA_LENGTH >> 8; 
                val[2] = RUN_STATE_DATA_LENGTH & 0xff; 
                ...
                val[7] = 0x01; 
                val[8] =0; 
                ak49xx_run_ram_write(codec->control_data, val) ;        
                } 
       ...
       ak4961_band_set=1; //flag the band set state
       return 0;
}
...
{ 
        ...
        {
        	pr_err("[lhs]%s: DSP not working, set later \n",__func__);
        }
...
//
	...
	ak4961->stream_state = AK4961_SLIMBUS_STREAM_NA;
       ak4961->pcm_state = AK4961_SLIMBUS_STREAM_NA;
